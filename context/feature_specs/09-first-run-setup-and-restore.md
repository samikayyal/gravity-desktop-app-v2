...
- `'gcp_enabled'`: Set to `'1'` if connectivity is validated successfully; otherwise `'0'`.
...
graph TD
    ...
    C -->|Skip / Next| E[Step 3: GCP Connectivity optional]
    E -->|Next / Skip| F[Step 4: Product Pre-fill]
    F -->|Finish| G[Step 5: Setup Completed -> Active Board]
```

### Setup Step Views
1. **Header & Progress Bar**: Standard banner with the trampoline brand styling. Displays progress indicators: `[1. Password] -> [2. Restore] -> [3. Cloud Test] -> [4. Catalog]`.
...
4. **Step 3: GCP Connectivity & Configuration (Optional)**:
   ...
   - Button `btnTestGcp` ("Test Cloud Connectivity") initiates background test.
   - Displays a success chip or a calm orange warning dialog: "Connectivity failed. Make sure your `.env` file contains correct GCP parameters."
...
| **SU-02** | GCP Offline | GCP credentials are missing, or internet is offline during setup. | **Allowed**. GCP connectivity test is fully skippable. Setting `'gcp_enabled'` is configured to `'0'` and setup continues normally. |
| **SU-03** | Safe Database Restore | Cashier uploads an invalid or corrupt SQLite database file. | **Strict rollback safety**. The system must: <br>1. Perform `PRAGMA integrity_check` on the uploaded file.<br>2. Check for the existence of required schema tables.<br>3. Copy the current draft database to a temporary location (`app_data/safety_copy.db`).<br>4. Attempt to replace the active database.<br>5. Roll back to `safety_copy.db` instantly if any exception is thrown. |
...
- **Safe Hot-Swap Database Restore**:
  - Write helper test `DatabaseRestoreHelper.verifyAndRestore(File uploadedFile)`:
    - **Scenario A (Valid SQLite)**: Upload a mock SQLite DB containing correct tables. Assert that restore completes, the active SQLite file matches the uploaded file, and a safety copy was generated.
...
- [ ] Database restore checks file integrity and runs rollback transactions to guarantee zero database corruption.
- [ ] GCP connectivity test is optional and skippable, ensuring the trampoline park works 100% offline.
- [ ] GCP connectivity status loads configurations strictly from local `.env` and never requests manually typed keys.
...
On launching the freshly installed trampoline park app, the cashier is presented with a clean full-screen wizard. They create the admin password `AdminGravity`. Clicking Next, they choose to skip restore. On Step 3, they click "Test GCP Connectivity"; the app queries the local `.env` variables, and shows a green checkmark indicating successful communication. On Step 4, they review the default stock of 100 grippy socks and 50 waters, click Finish, and are instantly redirected to the split-panel Active Board workspace, ready to check in their first player.
